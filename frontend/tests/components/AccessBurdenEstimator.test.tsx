...
describe('AccessBurdenEstimator', () => {
  it('renders with collapsed view showing estimate range', () => {
    render(<AccessBurdenEstimator distanceKm={20} province="ON" hospitalType="urban" />);
  ...
  it('displays prominent disclaimer about not delaying care', () => {
    ...
    expect(screen.getByText(/Never delay emergency care for cost/i)).toBeInTheDocument();
  ...
  it('expands to show detailed breakdown when clicked', () => {
    ...
    const button = screen.getByRole('button', { name: /expand access burden details/i });
  ...
  describe('fuel cost calculation', () => {
    ...
    it('uses correct gas price for British Columbia', () => {
      ...
      render(<AccessBurdenEstimator distanceKm={20} province="BC" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('uses correct gas price for Alberta', () => {
      ...
      render(<AccessBurdenEstimator distanceKm={20} province="AB" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('calculates round trip distance correctly', () => {
      render(<AccessBurdenEstimator distanceKm={15.5} province="ON" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
  ...
  describe('parking estimates', () => {
    ...
    it('shows suburban parking range', () => {
      render(<AccessBurdenEstimator distanceKm={20} province="ON" hospitalType="suburban" />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('shows rural parking range', () => {
      render(<AccessBurdenEstimator distanceKm={20} province="ON" hospitalType="rural" />);
      fireEvent.click(screen.getByRole('button'));
  ...
  describe('methodology information', () => {
    ...
    it('explains why this feature exists', () => {
      ...
      expect(screen.getByText(/Financial barriers.*often invisible/i)).toBeInTheDocument();
    });
  });
  ...
  describe('accessibility', () => {
    ...
    it('has descriptive aria-label', () => {
      ...
      expect(screen.getByRole('button', { name: /expand access burden details/i })).toBeInTheDocument();

      fireEvent.click(screen.getByRole('button'));
      expect(screen.getByRole('button', { name: /collapse access burden details/i })).toBeInTheDocument();
    });
  });

  describe('edge cases', () => {
    it('handles very short distances', () => {
      render(<AccessBurdenEstimator distanceKm={0.5} province="ON" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('handles unknown province with default gas price', () => {
      render(<AccessBurdenEstimator distanceKm={20} province="ZZ" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
    ...
    it('handles very long distances', () => {
      render(<AccessBurdenEstimator distanceKm={150} province="ON" hospitalType="urban" />);
      fireEvent.click(screen.getByRole('button'));
