...
**Immediate restart point (updated 2026-04-15):** the March 28 Neon transfer-quota outage is no longer the active blocker. Live verification on 2026-04-01 confirmed `/api/health` returning `healthy: true` with the database connected, `/api/hospitals` and `/api/resources` responding again, plus green `Heartbeat Monitor` and scheduled `Production Smoke` runs. Repo-side hardening for `/api/status`, `/api/data-quality`, quality scrape-window semantics, runtime env bootstrap, and heartbeat threshold alignment is complete, and the Playwright UI regression suite remains stabilized in the repo. The next concrete work items are now:

1. publish the latest VPS frontend release and verify that live `/api/status` and aggregate `/api/data-quality` now exclude inactive legacy source IDs and reflect the 4 active live sources; as of 2026-04-09, the live smoke check still fails on those two endpoints because the shared-VPS release step has not been run from a workstation with a reachable SSH target
2. complete the manual cost follow-up: move the production Neon project off free-tier fragility (Launch plan), with the Open Source Program as an optional credit path
3. keep Playwright browser verification CI-first and manual-dispatch on GitHub Actions to conserve free-tier minutes now that the suite itself is stable again
...
- [ ] **P2 / Ops: Batch frontend dependency refresh** — Closed Dependabot PRs #38 through #48 rather than drip-merging them; revisit frontend package updates in one deliberate lockfile refresh with a coherent lockfile/tooling plan
...
- [ ] **P1 / Ops: Release and verify the hardened public status/data-quality summaries on the VPS** — Repo-side filtering and smoke coverage landed on 2026-04-01. Live smoke on both 2026-04-09 and 2026-04-16 still showed `manitoba-shared-health` leaking through `/api/status` plus aggregate `/api/data-quality`, confirming the shared-VPS frontend is still serving an older release. The remaining step is to deploy the current frontend release to the shared VPS from a workstation with a reachable SSH target and confirm those live summaries no longer expose legacy source IDs or stale critical rollups
- [ ] **P1 / Ops: Upgrade the production Neon project off the free tier** — Manual external change. The free tier already produced a transfer-quota outage on 2026-03-28 and an 89% storage warning on 2026-03-31; move to Launch, then monitor a billed month before revisiting self-hosting
