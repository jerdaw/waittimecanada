...
test.describe('Hospital Map', () => {
  ...
  test('should show hospital details when clicking a marker', async ({ page }) => {
    ...
    await expect(page.locator('h3').filter({ hasText: 'Test General Hospital' })).toBeVisible();
  ...
  test('should toggle comparison mode', async ({ page }) => {
    ...
    await expect(page.getByText('Select 2 hospitals to compare (0/2 selected)')).toBeVisible();
