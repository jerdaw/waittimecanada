...
const EQUITY_SUMMARY_CACHE_TTL_MS = 300_000;
...
export async function GET(request: Request) {
  ...
  try {
    const payload = await getOrSetServerCache(
      buildServerCacheKey("api:analytics:equity-summary", {
        province,
        period: periodConfig.label,
      }),
      EQUITY_SUMMARY_CACHE_TTL_MS,
      async () => {
        ...
        const summary = computeEquityLinkageSummary(
          hospitalPoints,
          equityData,
          30,
        );
