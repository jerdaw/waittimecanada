...
| **Double Footer**       | `layout.tsx` renders `<Footer />` globally. Homepage _also_ renders `<Footer />` conditionally when `!showHero`. This means the footer appears twice after hero dismissal. Other pages (analytics, data-quality, etc.) render their own `<Footer />` too — tripling it in some cases. |
