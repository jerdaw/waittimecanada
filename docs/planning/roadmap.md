...
- [x] **P1 / Public Health Hub: Ontario selective expansion (2026-04-23)** — Added Ontario-scoped ISC long-term drinking water advisories to `/resources`, aligned the advisory freshness window to the observed ISC publication cadence, and extended production smoke/docs for the new route
- [x] **P1 / Public Health Hub: Restore ODHF live ingest health (2026-04-23)** — Operationalized the live Statistics Canada ZIP ingest, fixed ODHF duplicate-ID handling for the real payload shape, and returned `public_health_hub_status` to overall `healthy`
...
- [x] **P2 / Ops: Wait for localized frontend routes after deploy (2026-04-23)** — `scripts/deploy-vps-frontend.sh` now waits for private `/api/health`, `/`, and `/methods` to return `200` before returning control to smoke/verification so restart-race false `502`s do not leak into routine deploy checks
...
- `docs/planning/archive/maintenance-2026-04-23.md` — Ontario public-health production closeout, docs reconciliation, and repo hygiene sweep (2026-04-23)
...
| [0023](../adr/0023-public-health-hub-module-boundary.md) | Keep the public-health hub Ontario-first, provenance-first, and separate from live wait-time claims |
| [0024](../adr/0024-ontario-naloxone-link-out-posture.md) | Keep Ontario naloxone guidance as official link-out only until reuse rights are clearer |
