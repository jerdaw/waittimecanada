...
- Production database freshness: `/api/health` is healthy with all four source heartbeats refreshed at `2026-07-09T02:39Z` after the latest manual scraper fallback.
...
- Recurrence verification: GitHub Actions created a successful heartbeat `event=schedule` run at `2026-07-09T01:15Z`; scraper schedule creation is still not proven, with no new scraper `event=schedule` runs observed through the `2026-07-09T02:29Z` slot. Manual scraper dispatch remains the live fallback until a new scraper `event=schedule` run is observed.
...
| Scraper scheduled event creation after restore | Workflow reports `active`, Actions permissions are enabled, and cron syntax is present on `main`, but no new scraper `event=schedule` runs appeared for observed restored windows through `2026-07-09T02:29Z` | current | high | keep manual scraper fallback active; confirm the first new scraper schedule-created run or move the cadence to a trusted external runner/scheduler |
| Post-scrape aggregate timeout | Manual scraper run `28990136047` completed scraper writes, but the workflow was cancelled when `Refresh current analytics aggregates` exceeded the 20-minute job timeout | mitigated | medium | freshness evidence now runs before aggregation; aggregate refresh is daily-only, step-bounded, and can be skipped for emergency manual freshness dispatch |
...
| Local production smoke | `PRODUCTION_BASE_URL=https://wait-time.ca bash scripts/production-smoke.sh` at `2026-07-09T02:59Z` | passed all public route/API checks and found no dormant legacy source IDs |
...
| Workflow guardrail check | `python3 scripts/check-scraper-workflow.py` | passes after mitigation; fails if scraper freshness evidence is placed after aggregate refresh, aggregate refresh lacks its own timeout, or emergency aggregate skip is removed |
...
| --- | --- | --- | --- |
| `/api/health` | 200, healthy | `last_update=2026-07-09T02:39:47.923Z`; source ages about 19-20 minutes at the `2026-07-09T02:58Z` check; source counts: AB 22, BC 11, ON 166, QC 192 | current scraper freshness recovered |
| `/api/status` | 200, critical | `system_uptime_24h=0.167`; `scheduler_cadence=hourly`; `expected_runs_24h=24`; all source heartbeats healthy | current freshness is healthy, but the 24-hour success-rate window is still recovering from the long gap |
| `/api/data-quality` | 200, critical | `system_uptime_24h=0.167`; `total_measurements_24h=1601`; all source heartbeats healthy | measurements are flowing again, but the 24-hour data-quality success-rate window needs recurring hourly runs to recover |
