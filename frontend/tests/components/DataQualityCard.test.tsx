...
describe("DataQualityCard", () => {
  ...
  it("shows success rate with green color for healthy", () => {
    render(
      <DataQualityCard source={makeSource({ last_24h_success_rate: 0.97 })} />,
    );
  ...
  it("shows success rate for degraded performance", () => {
    render(
      <DataQualityCard source={makeSource({ last_24h_success_rate: 0.85 })} />,
    );
  ...
  it("shows success rate for critical performance", () => {
    render(
      <DataQualityCard source={makeSource({ last_24h_success_rate: 0.5 })} />,
    );
  ...
  it("shows heartbeat age", () => {
    render(
      <DataQualityCard
        source={makeSource({ last_heartbeat_age_minutes: 5 })}
      />,
    );
  ...
  it("shows 'Never' when heartbeat is null", () => {
    render(
      <DataQualityCard
        source={makeSource({ last_heartbeat_age_minutes: null })}
      />,
    );
