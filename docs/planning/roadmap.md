...
**Immediate restart point (updated 2026-04-01):** the March 28 Neon transfer-quota outage is no longer the active blocker. Live verification on 2026-04-01 confirmed `/api/health` returning `healthy: true` with the database connected, `/api/hospitals` and `/api/resources` responding again, plus green `Heartbeat Monitor` and scheduled `Production Smoke` runs. The next concrete work items are now:

1. reconcile the live VPS frontend release / public summary surfaces: `/api/status` and `/api/data-quality` still report `overall_status: critical` and include inactive legacy source IDs (`manitoba-shared-health`, `on-health`) despite healthy live runs for the 4 active sources
2. stabilize the Playwright UI regression suite
3. complete the manual cost follow-up: move the production Neon project off free-tier fragility (Launch plan), with the Open Source Program as an optional credit path
...
- [x] **P1 / Ops: Refresh GitHub Actions security workflow dependencies** — Reconciled on 2026-03-28. The only open trace was stale roadmap wording; the live `Scraper CI` security job already runs through the current upstream `PyCQA/bandit-action@v1` composite, which now uses `actions/checkout@v6`, `actions/setup-python@v6`, and `github/codeql-action/upload-sarif@v4`. No repo-side workflow change was required beyond documentation cleanup.
...
- [ ] **P1 / Ops: Reconcile public status/data-quality summaries with the recovered live source set** — As of 2026-04-01, `/api/status` and `/api/data-quality` still report `overall_status: critical` and include inactive legacy source IDs (`manitoba-shared-health`, `on-health`) despite healthy live runs for the 4 active sources; determine whether the VPS frontend release is stale or the public summary logic/data still needs follow-up
