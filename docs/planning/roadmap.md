...
1. monitor Neon transfer/storage through the first billed Launch window and keep the current cache/retention guardrails in place; treat the Open Source Program only as an optional credit path
...
- [ ] **P2 / Ops: Monitor first billed month on Neon Launch** — Watch transfer/storage usage through the active Launch billing window, keep the current frontend read-cache and bounded-cleanup guardrails in place, and only reopen deeper DB-path optimization or self-hosting if cost/reliability pressure remains meaningful on Launch
