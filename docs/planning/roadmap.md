...
- [x] **P1 / Ops: Refresh GitHub Actions security workflow dependencies** — Reconciled on 2026-04-01 by replacing the deprecated `PyCQA/bandit-action@v1` composite with an explicit Bandit run plus `github/codeql-action/upload-sarif@v4`, removing the remaining Node 20 `upload-sarif@v3` warning path from `Scraper CI`
