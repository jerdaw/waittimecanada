...
- Source-health summary and freshness badge generation run immediately after
  ...
  separate 15-minute step timeout.
- Manual dispatch supports `refresh_analytics=false` for emergency
  freshness-only recovery when aggregate refresh is suspected slow.
