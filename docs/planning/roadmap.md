...
- [ ] **P1 / Ops: Release and verify the hardened public status/data-quality summaries on the VPS** — Repo-side filtering and smoke coverage landed on 2026-04-01, and live smoke on 2026-04-09 still showed `manitoba-shared-health` leaking through `/api/status` plus aggregate `/api/data-quality`. The remaining step is to deploy the current frontend release to the shared VPS from a workstation with a reachable SSH target and confirm those live summaries no longer expose legacy source IDs or stale critical rollups
