...
**Immediate restart point (recorded 2026-03-28):** production is paused behind a real external blocker. Neon is currently rejecting database connections with `Your project has exceeded the data transfer quota. Upgrade your plan to increase limits.` This is causing live DB-backed routes such as `/api/hospitals`, `/api/resources`, `/api/status`, and `/api/data-quality` to return `500`, and the `Heartbeat Monitor (Dead Man's Switch)` workflow is failing for the same reason. Before any further roadmap execution, resolve or wait out the Neon quota issue, then rerun production verification:

1. confirm live `/api/health` reports the database as connected again
2. rerun production smoke/readiness
3. confirm heartbeat monitor recovery
4. then resume with the next open engineering item, currently `P1 / Ops: Stabilize Playwright UI regression suite`
...
- [ ] **P0 / Ops: Restore production DB connectivity after Neon transfer quota exhaustion** — External blocker recorded on 2026-03-28. Live DB-backed routes are returning `500` and the heartbeat monitor is failing because Neon is rejecting connections with `Your project has exceeded the data transfer quota.` Resume work only after quota/reset or production DB-path changes restore connectivity, then rerun smoke/readiness and confirm recovery.
