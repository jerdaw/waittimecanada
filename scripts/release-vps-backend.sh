...
usage() {
  cat <<'EOF' >&2
usage: scripts/release-vps-backend.sh <ssh-target> [--deploy]

Creates a release from the current committed tree, uploads it to the VPS,
repoints /srv/apps/waittime-backend/current, and optionally runs the
backend VPS deploy script.

Environment overrides:
  WAITTIME_BACKEND_VPS_APP_ROOT   default: /srv/apps/waittime-backend
  WAITTIME_BACKEND_VPS_ENV_FILE   default: /etc/projects-merge/env/waittime-backend.env
EOF
  exit 1
}
...
if [[ "$deploy_after_release" == "true" ]]; then
  ...
  ssh "$ssh_target" "
    cd '$app_root/current' &&
    ./scripts/deploy-vps-backend.sh '$env_file'
  "
fi
