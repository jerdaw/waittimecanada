...
def main() -> int:
    ...
    for source_id in TRUSTED_SOURCE_IDS:
        ...
        needs_approval = [
            h for h in hospitals if not h.is_verified or not h.is_visible
        ]
        ...
        if not needs_approval:
            ...
            logger.info(
                f"{source_id}: {already} hospital(s) already approved, 0 to update"
            )
            continue
        ...
        if args.dry_run:
            ...
            logger.info(
                f"{source_id}: {len(needs_approval)} hospital(s) would be approved"
            )
        else:
            for h in needs_approval:
            ...
            logger.info(
                f"{source_id}: {len(needs_approval)} hospital(s) approved"
            )
    ...
    logger.info(
        f"Done: {action} {total_approved} hospital(s), "
        f"{total_already} already approved"
    )
    return 0
